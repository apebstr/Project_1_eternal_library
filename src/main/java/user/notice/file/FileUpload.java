...
public class FileUpload {
	...
	public static UUID saveCopyFile(MultipartFile file, HttpServletRequest request) {
		...
		//System.out.println("첨부파일 저장경로 :"+ ff);
		try {
	...
	public static UUID saveCopyImg(MultipartFile img, HttpServletRequest request) {
		...
		//System.out.println("첨부사진 저장경로 :"+ ff);
		try {
	...
	//첨부파일이 저장될 위치 설정 (/resources/assets/temp)
	public static String urlPath(HttpServletRequest request) {
		//서버에 저장할 위치값
		String root = request.getSession().getServletContext().getRealPath("/resources/assets/");
